...
if ! command -v python3 &>/dev/null; then
    ...
    echo "Instale com: apt install python3 python3-venv"
    exit 1
fi
...
if ! command -v psql &>/dev/null; then
    ...
    echo "Instale com: apt install postgresql postgresql-client"
    exit 1
fi
...
if ! pg_isready -q 2>/dev/null; then
    ...
    sleep 2
    if ! pg_isready -q 2>/dev/null; then
        echo "[AVISO] Nao foi possivel verificar o PostgreSQL. Verifique manualmente."
    else
...
if [ ! -f "$DESTINO/.env" ]; then
    ...
    echo ""
    echo "============================================"
    echo "  ATENCAO: Edite o arquivo $DESTINO/.env"
